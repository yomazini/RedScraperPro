...
# TODO: to  Migrate into the Core Scrapper this advanced veto sentiment analysis and also to add the HF https://huggingface.co/tabularisai/multilingual-sentiment-analysis Top 25% ;
...
class SentimentAnalyzer:
        ...
        if self.textblob_available:
            try:
                ...
                sentiment_data["textblob"] = {
                    ...
                    "classification": self._classify_textblob_sentiment(
        ...
        if self.vader_available:
            try:
                ...
                sentiment_data["vader"] = {
                    ...
                    "classification": self._classify_vader_sentiment(
                        scores["compound"]
                    ),
                }
            except Exception as e:
    ...
    def analyze_batch(self, texts: List[str]) -> List[Optional[Dict[str, Any]]]:
        ...
        for i, text in enumerate(texts):
            try:
                ...
                if (i + 1) % 100 == 0:
                    self.logger.debug(
                        f"Processed {i + 1}/{len(texts)} texts for sentiment analysis"
                    )
    ...
    def analyze_reddit_data(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, item in enumerate(data):
            try:
                ...
                if (i + 1) % 50 == 0:
                    self.logger.debug(
                        f"Sentiment analysis progress: {i + 1}/{len(data)}"
                    )
    ...
    def _extract_text_from_item(self, item: Dict[str, Any]) -> str:
        ...
        if item.get("type") == "post":
            ...
            return (
                item.get("content", "") or item.get("body", "") or item.get("title", "")
            )
    ...
    def _combine_sentiments(self, sentiment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        combined = {"classification": "neutral", "confidence": 0.0, "agreement": False}
